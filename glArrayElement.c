...
static const
GLfloat scale_factor [] = { 1.0f/127.0f, 1.0f/255.0f, 1.0f/32767.0f, 1.0f/65535.0f, 1.0f/2147483647.0f, 1.0f/4294967295.0f };


static
void dereference (struct pspgl_vertex_array *a, GLint idx, void (*glfunc) (const GLfloat *v), int scale)
{
	if (a->enabled) {
		const void *ptr = (const void *) ((unsigned long) a->ptr + a->stride * idx);
		GLfloat val [4];
		GLfloat factor;
		int i;
